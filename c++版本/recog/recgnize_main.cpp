/*************************************************************************
    ...
    > Created Time: 2019年04月02日 星期二 10时56分12秒
 ************************************************************************/
...
/*	
			...
			{
				case 0:
					putText(frame, "youjiang", faces[i].tl(), FONT_HERSHEY_PLAIN, 1.0, Scalar(0, 255, 0), 1, 30);//在人脸旁显示人名
					break;
				case 1:
					putText(frame, "miaobo", faces[i].tl(), FONT_HERSHEY_PLAIN, 1.0, Scalar(0, 255, 0), 1, 30);
					break;
				default:
					putText(frame, "un", faces[i].tl(), FONT_HERSHEY_PLAIN, 1.0, Scalar(0, 255, 0), 1, 8);
					break;
...
int main(int argc, char**argv)
{
	...
	if (argv[1] == cmd1){
		...
		save_FaceSamples(100);
		//runPy_getcsv();         //下面用ｃ＋＋写的代替ｐｙｔｈｏｎ
		get_namescsv();
		start_train(1);  // 1 强制训练
		std::cout << "train ok!"<<std::endl;
	}
	else if(argv[1] == cmd2){
